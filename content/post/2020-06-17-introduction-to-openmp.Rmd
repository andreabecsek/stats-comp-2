...
OpenMP is an interface that provides support for parallel programming. 

The easiest thing we can do is create a simple C++ code, compile it and run it multiple times in parallel.
...
Note that we included a single new line, `#pragma omp parallel` which simply mentions that the code between the brackets is what we aim to parallelize.

And we compile the program using
```{bash eval=FALSE}
...
Reductions combine multiple subcalculations into one after every calculations. This way we don't have to store all the results until the end.
